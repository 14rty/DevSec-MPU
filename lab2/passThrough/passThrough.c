...
--*/
{
    ...
    PFLT_FILE_NAME_INFORMATION NameInfo = NULL;// ќбъ€вление структуры
    status = FltGetFileNameInformation(
        Data,
        FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT,
        &NameInfo
    );// ѕарсинг имени файла на составл€ющие

    UNICODE_STRING requiredExtension = RTL_CONSTANT_STRING(L"boom");// ѕеревод расширени€ в UNICODE_STRING
    if (NT_SUCCESS(status)) {
        if (RtlEqualUnicodeString(&requiredExtension, &NameInfo->Extension, FALSE)) {
            if (Data->Iopb->MajorFunction == IRP_MJ_READ) {//проверка на событие записи
                ...
                if (strlen((char*)Data->Iopb->Parameters.Read.ReadBuffer) != 0) {
                    ...
                    AES_init_ctx_iv(&ctx, key, iv); //инициализаци€ структуры
            ...
            else if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) { //проверка на событие чтени€
